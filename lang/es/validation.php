...
/**
 * Spanish translation of Laravel's default validation messages. Without this
 * file, any native rule not explicitly overridden in a FormRequest::messages()
 * (e.g. `email`, `required`, `max`) falls back to the raw translation key
 * (e.g. "validation.email") because there is no default-locale fallback file
 * to catch it. This file is the general fix: it covers every native rule, so
 * new fields/rules added later keep working in Spanish without touching this
 * file again. Request-specific overrides (see lang/es/contact-form.php) keep
 * priority over these defaults — Laravel resolves the FormRequest::messages()
 * array before falling back to this file.
 *
 * Keys mirror vendor/laravel/framework/.../Translation/lang/en/validation.php
 * (Laravel 12). Keep both files in sync if the framework adds new rules.
 */
return [
    ...
    |--------------------------------------------------------------------------
    |
    | Prefer request-specific overrides in the corresponding form's language
    | file (e.g. lang/es/contact-form.php) over adding entries here, so the
    | copy stays next to the feature that owns it.
    |
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Prefer FormRequest::attributes() overrides (see contact-form.php) over
    | adding entries here.
    |
    */
